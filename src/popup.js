...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function loginWithGoogle() {
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${API_CONFIG.BASE_URL}/auth/success`,
        },
      });
    ...
    try {
      ...
      try {
        ...
        notificationManager.showError(
          getUserFriendlyErrorMessage(error, "stats")
        );
  ...
  async function loadMembershipStatus(accessToken) {
    try {
      ...
      notificationManager.showError(
        getUserFriendlyErrorMessage(error, "membership")
      );
  ...
  function displayMembershipStatus(userData) {
    ...
    const isPro =
      (userData.subscription?.status === "pro" ||
        userData.subscription?.membershipStatus === "pro") &&
